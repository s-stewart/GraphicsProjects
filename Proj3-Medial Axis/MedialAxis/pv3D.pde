...
vec U(vec V) {float n = V.norm(); if (n<0.0000001) return new vec(0,0,0); else return new vec(V.x/n,V.y/n,V.z/n);};             // V/||V||
...
pt R(pt Q, pt C, pt P, pt R) { // returns rotated version of Q by angle(CP,CR) parallel to plane (C,P,R)
   ...
   float x=d(V,I0), y=d(V,J0);
                                //  stroke(red); show(C,400,I0); inverse
                                stroke(blue); show(C,400,I1); stroke(orange); show(C,400,J0); stroke(magenta); show(C,400,J1); noStroke();
