...
class vec { float x=0,y=0,z=0; 
   ...
   vec (pt p) { x = p.x; y = p.y; z = p.z; }
   vec (vec p) { x = p.x; y = p.y; z = p.z; }
   ...
   void setComponent(int i, float val) {
     if(0 == i) x = val;
     if(1 == i) y = val;
     if(2 == i) z = val;
   }
...
String toText(pt p){ return "("+nf(p.x,1,5)+","+nf(p.y,1,5)+","+nf(p.z,1,5)+")"; }
// ==== curve
